...
\item The space of possible values for $s_{1:m}$, i.e., $S^m$. 
\item The normalization constant (denominator in the above expression) involves a sum over the set $S^m$.
...
\item  The next question is how to define $\vec{\Phi}(x_{1:m},s_{1:m})$? 

\begin{displaymath}
 \vec{\Phi}(x_{1:m},s_{1:m}) = \sum_{j=1}^{m} \vec{\phi}(x_{1:m},j,s_{j-1},s_j) 
\end{displaymath}

where $\vec{\phi}(x_{1m},j,s_{j-1},s_j)$ are the same as the feature vectors used in MEMMs.
...
\begin{itemize}
\item We are assuming that for $k= 1, \dots, d$, the $k$'th global feature is:
