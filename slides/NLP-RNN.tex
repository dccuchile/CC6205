...
\begin{displaymath}
\text{score}_{lstm-crf}(x_{1:m},s_{1:m})= \sum_{i=1}^{m} \left(W_{[s_{i-1},s_i]} + \text{LSTM}^*(x_{1:m})_{[i]} \right) 
\end{displaymath}

where $W_{[s_{i-1},s_i]}$ corresponds to a transition score from tag $s_{i-1}$ to $s_i$  that is learned by the network and $\text{LSTM}^*(x_{1:m})_{[i]}$ comes from the hidden state of the Bi-LSTM at timestep $i$.
