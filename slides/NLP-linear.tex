...
\begin{itemize}
 ...
 \end{displaymath}
\item The predicted label distribution of the categorical cross-entropy loss ($\vec{\hat{y}}$) is obtained by applying the softmax function the last layer of the network $\vec{\tilde{y}}$:
    \begin{displaymath}
\vec{\hat{y}}_{[i]} = \text{softmax}(\vec{\tilde{y}})_{[i]} =  \frac{e^{\vec{\tilde{y}}_{[i]}}}{\sum_{j}e^{\vec{\tilde{y}}_{[j]}}}   
 \end{displaymath}
