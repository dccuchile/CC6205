...
\begin{scriptsize}
\begin{itemize}
\item All the parameters are initialized with random values ($W, \vec{b}$).
\item For each training example $(x,y)$ we calculate the loss $L$ with current values of $w$.
\item Then we update the parameters with the following rule until convergence:
\item $w_i \leftarrow w_i - \eta \frac{\partial L}{w_i}(x,y)$  (for all parameters $w_i$)
