...
\begin{frame}{Backpropagation (without vectorial derivatives)}
\begin{scriptsize}


\begin{itemize}
\item In a general feed-forward netowrk, each unit computes a weighted sum of its inputs in the form:
\begin{equation}
\vec{h}_{[j]}^l = \sum_{i}  W_{[i,j]}^l \times \vec{z}_{[i]}^{(l-1)} + \vec{b}_{[j]}^l
\end{equation}
...
\begin{equation}
\vec{\delta}_{[j]}^l = \frac{\partial L}{\partial \vec{h}_{[j]}^l}
\end{equation}
...
\begin{equation}
\vec{\delta}_{[j]}^l = \frac{\partial L}{\partial \vec{h}_{[j]}^l} = \sum_{k,m (m>l)} \frac{\partial L}{\partial \vec{h}_{[k]}^m} \times \frac{\partial \vec{h}_{[k]}^m}{\partial \vec{h}_{[j]}^l}
\end{equation}
...
\item where z i is the activation of a unit, or input, that sends a connection to unit j, and w ji is the weight associated with that connection. superscripts correpond to layer indexes
