...
Un n-grama es una secuencia contigua de $n$ palabras. Un modelo de lenguaje de trigrama (n-grama con $n=3$) es una implementación directa del proceso de Markov de segundo orden que se discutió anteriormente. Este modelo está compuesto por:
...
Aquí, definimos $x_0 = x_{-1} = *$ como símbolos especiales por conveniencia.
...
Un buen modelo de lenguaje debe asignar probabilidades más altas a oraciones coherentes que a oraciones sin sentido. Para evaluar la calidad de un modelo de lenguaje, es crucial no utilizar el mismo corpus en el que fue entrenado, ya que el objetivo es verificar cómo generaliza a nuevas oraciones. La evaluación cuantitativa de modelos de lenguaje es una tarea fundamental en PLN, ya que nos permite comparar distintos enfoques y seleccionar el modelo más apropiado.

Por lo tanto, es común utilizar un corpus de prueba independiente, $\mathcal{C}_{\text{test}}$, que contiene $m$ oraciones: $s_1, s_2, s_3, ..., s_m$, y cuantificar la probabilidad que el modelo asigna a estas oraciones. Para esto, calculamos la probabilidad total asignada por el modelo a las oraciones del corpus de prueba, lo cual se puede expresar más convenientemente mediante la probabilidad logarítmica:
\[
...
La perplejidad tiene la propiedad de ser más fácil de interpretar que la probabilidad logarítmica, como se discutirá a continuación.
