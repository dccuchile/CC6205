...
Un modelo de lenguaje de trigrama es una implementación directa del proceso de Markov de segundo orden y consiste en:
...
Abordemos ahora el problema de la estimación de los parámetros $q(w_i | w_{i-2}, w_{i-1})$ a partir de un corpus $\mathcal{C}$.


Una estimación natural es la ``estimación de máxima verosimilitud'' (MLE), la cual es es una técnica común para estimar los parámetros de modelo estadísticos donde se estiman los parametros para maximizar la probabilidad de los datos de entrenamiento. 
...
Esto requiere tokenizar el corpus de entrenamiento y guardar en una tabla los conteos de todos los trigramas y bigramas encontrados. 
...
Recordemos que un buen modelo de lenguaje debiese asignarle probabilidades mayores a oraciones fluídas que a oraciones carented de sentido. 

Tenemos algunos datos de prueba, $m$ oraciones: $s_1, s_2, s_3, ..., s_m$. Podemos analizar la probabilidad bajo nuestro modelo $\prod_{i=1}^{m} p(s_i)$. O más convenientemente, la probabilidad logarítmica:
    \[
        \log \left( \prod_{i=1}^{m} p(s_i) \right) = \sum_{i=1}^{m} \log p(s_i)
    \]

De hecho, la medida de evaluación habitual es la perplejidad (o perplexity):
    \[
...
\textbf{Demostración:} Supongamos que tenemos $m$ oraciones de longitud $n$ en el corpus, y $M$ es la cantidad de tokens en el corpus, $M = m \cdot n$. Consideremos el logaritmo (base 2) de la probabilidad de una oración $s = w_1 w_2 \dots w_n$ bajo el modelo:
    \[
...
Una limitación de este enfoque tiene que ver con la dispersión del lenguaje (Capítulo~\ref{cap_intro} lo que hace muy grande la cantidad de parámetros a estimar, de hecho si el tamaño del vocabulario es $N = |V|$, entonces hay $N^3$ parámetros en el modelo. Por ejemplo, si $N = 20,000$, entonces $20,000^3 = 8 \times 10^{12}$ parámetros. Veremos más adelante
...
Sin embargo, los modelos de lenguaje basados en MLE pueden sufrir de overfitting (sobreajuste) a los datos de entrenamiento.
...
Para abordar el problema del overfitting en modelos de lenguaje, se utilizan diversas técnicas de regularización. 
Estas técnicas de regularización ayudan a controlar la varianza del modelo y a mitigar el overfitting, permitiendo un mejor equilibrio entre el sesgo y la varianza y mejorando la generalización a nuevas muestras. A continuación veremos dos técnicas de regularización para modelos de lenguaje, la interporlación lineal y los modelos de descuento. 
