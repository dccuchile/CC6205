...
\vspace{-0.5cm}
\title{Natural Language Processing \\ 05. Linear Models}
\vspace{-0.5cm}
...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=0.26]{pics/supervised.png}
\end{figure}
\footnotetext[1]{Source: \cite{AbuMostafa2012}}
\end{frame}
...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=0.4]{pics/dogs_vs_cats1.png}
\end{figure}
\end{frame}

\begin{frame}{Classification problem: linear model}
\begin{figure}[htb]
    \centering
    \includegraphics[scale=0.4]{pics/dogs_vs_cats2.png}
\end{figure}
\end{frame}

\begin{frame}{Classification problem: other alternatives}
\begin{figure}[htb]
    \centering
    \includegraphics[scale=0.4]{pics/dogs_vs_cats3.png}
\end{figure}
\end{frame}
...
\begin{frame}{Classification problem: linear model (several alternatives)}
\begin{figure}[htb]
    \centering
    \includegraphics[scale=0.4]{pics/dogs_vs_cats4.png}
\end{figure}
\end{frame}
...
\end{equation}
where $\#ab$ is the number of times the bigram $ab$ appears in the document, and $|D|$ is the total number of bigrams in the document (the document’s length - 1).
...
\begin{figure}[htb]
	\centering
	 \includegraphics[scale=0.2]{pics/langbigrams.png}
\end{figure}
...
\footnotetext[2]{\scriptsize{Source:\cite{goldberg2017neural}}}
...
\item Usually, we set a threshold $T = 0.5$. If $P(\hat{y} | \vec{x}) > T$, then we say that $\hat{y} = 1$, 0 otherwise. \textbf{Very important:} $T=0.5$ is just the default threshold, but it can (and must!) be adjusted depending on the problem you are trying to solve; sometimes a false negative is more important than a false positive or vice versa.
\end{itemize}
\end{scriptsize}
\end{frame}
...
\begin{scriptsize}
\begin{itemize}
    \item From Eq. 9, we know that $\vec{\hat{y}} = f(\vec{x}) = \vec{x} \cdot W + \vec{b}$
    \item Let's forget for a minute about the biases $\vec{b}$ and focus only on $\vec{x} \cdot W$.
    \item Since $\vec{x}$ can be decomposed into a sum of one-hot vectors:
    \begin{equation}
    \begin{aligned}
        \vec{\hat{y}} &= \vec{x} \cdot W \\ 
        &= \Bigl(\frac{1}{|D|} \sum_{i=1}^{|D|}\vec{x}^{|D|_{[i]}} \Bigr) \cdot W \\
        &= \frac{1}{|D|} (\sum_{i=1}^{|D|}\vec{x}^{|D|_{[i]}} \cdot W)  \\
        &= \frac{1}{|D|} \sum_{i=1}^{|D|}W^{|D|_{[i]}}
    \end{aligned}
    \end{equation}
    \item In other words, the representation of the document can be understood as a sum of word representations. This is called \textit{continous bag of words} (CBOW), and it will be revisited in a few classes from now :eyes:
\end{itemize}
\end{scriptsize}
\end{frame}
...
\section{Regularization and hyperparameters}
