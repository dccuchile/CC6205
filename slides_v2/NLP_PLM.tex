...
\vspace{-0.5cm}
\title{Natural Language Processing \\ 04. Probabilistic Language Models}
\vspace{-0.5cm}
...
\end{enumerate}
\item These slides are adapted from Felipe Bravo-Marquez' work, which itself was based on the course material by Michael Collins: \url{http://www.cs.columbia.edu/~mcollins/cs4705-spring2019/slides/lmslides.pdf} 
...
\begin{itemize}
    \item Language Model: a function that assigns probabilities to any given sentence

    \item \textit{Good} language model: the model assigns higher probabilities to phrases that are meaningful and fluent
\end{itemize}
\end{frame}

\begin{frame}{Why?}
    \begin{figure}[h]
    \includegraphics[scale = 0.2]{img/wrong_caption.png}
\end{figure}
\end{frame}
...
\item Consider the sentences: 1) \textit{recognize speech} and 2) \textit{wreck a nice beach}.
...
\begin{figure}[h]
\includegraphics[scale = 0.6]{img/lmsampling.png}
\end{figure}
...
\begin{figure}[h]
\includegraphics[scale = 0.2]{img/bias_variance.png}
\end{figure}


\footnotetext[1]{Image source: \href{https://datamapu.com/posts/ml_concepts/bias_variance/}{\textit{Datamapu} - Bias and Variance}}
